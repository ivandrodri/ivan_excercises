...
import numpy as np
import examples.offline_RL_workshop.custom_envs.custom_2d_grid_env.rendering as r
from examples.offline_RL_workshop.custom_envs.custom_2d_grid_env.window import Window
...
class SimpleGridEnv(Env):
    ...
    def __init__(self,
        ...
        if self.discrete_action:
            self.action_space = spaces.Discrete(len(self.MOVES))  # Two possible actions: move left or move right
        ...
        self.observation_space = spaces.Box(low=0., high=1.0, shape=(self.nrow*self.ncol,), dtype=np.float64)
        ...
        # ToDo: Hardocode --> change it soon!
        #if start_location is None:
        start_location = (0,0)
        #if target_location is None:
        target_location = (self.ncol-1,self.nrow-1)
        #target_location = (self.ncol - 1, 0)
    ...
    def integrity_checks(self) -> None:
        ...
        assert self.obstacles[tuple(self.start_xy)] == self.FREE, \
            f"Start position {self.start_xy} overlaps with a wall."
        assert self.obstacles[tuple(self.goal_xy)] == self.FREE, \
            f"Goal position {self.goal_xy} overlaps with a wall."
        assert self.is_in_bounds(*tuple(self.start_xy)), \
            f"Start position {self.start_xy} is out of bounds."
        assert self.is_in_bounds(*tuple(self.goal_xy)), \
    ...
    def get_reward(self, x: int, y: int) -> float:
        ...
        if not self.is_in_bounds(x, y):
            ...
            return -0.1*((self.goal_xy[0]-x)**2 + (self.goal_xy[1]-y)**2)**0.5/(self.ncol*self.nrow)
