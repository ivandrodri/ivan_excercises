...
def create_cql_continuous_policy_from_dict(policy_config:Dict[str,Any], action_space:gym.core.ActType,
    ...
    critic1 = tianshou.utils.net.continuous.Critic(net_c1, device=policy_config["device"]).to(policy_config["device"])
    critic1_optim = torch.optim.Adam(critic1.parameters(), lr=policy_config["critic_lr"])
    critic2 = tianshou.utils.net.continuous.Critic(net_c2, device=policy_config["device"]).to(policy_config["device"])
